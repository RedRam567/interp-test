...
async fn main() {
    ...
    game_state.tick_number = 281474976710656;
    game_state.init();
...
fn dbg_info(game: &GameState, global_state: &GlobalState, t: f32) {
    ...
    draw_text(&format!("fps: {: >4}, frame time: {:.9}s", get_fps(), get_frame_time()), 0.0, next_line(), TYPEFACE_SIZE, WHITE);
    ...
    draw_text(&format!("tick: {: >5}, game time: {}:{:0>2} or {}{}s", tick, gametime_whole, gametime_ticks, gametime_s, gametime_frac), 0.0, next_line(), TYPEFACE_SIZE, WHITE);
...
//     draw_text(version, 0.0, next_line(), TYPEFACE_SIZE, WHITE);
// }
